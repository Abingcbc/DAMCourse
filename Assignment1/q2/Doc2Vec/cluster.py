...
user_id = pd.read_csv("feature_doc2vec.csv").values[:,0]
X = pd.read_csv("feature_doc2vec.csv").values[:,1:]
...
# Density Peak 0.477733216224982 23931.195463737473
print("----- Density Peak start -----")
start = time.time()
labels_dp = density_peak.predict(X_new, 0.02, 4)
class_count_dp = []
for i in range(6):
...
# K-Means 0.5251180546243777 36934.27843173479
print("----- K-Means start -----")
start = time.time()
cluster_km = KMeans(n_clusters=4)
...
#DBSCAN -0.03385949219699722 907.8335095008497
print("----- DBSCAN start -----")
start = time.time()
cluster_db = DBSCAN(eps=0.01, min_samples=47)
...
#Hierarchical Clustering 0.5422762118339837 27411.38806333127
print("----- Hierarchical Clustering start -----")
start = time.time()
cluster_hc = AgglomerativeClustering(n_clusters=4)
...
#Spectral Clustering 0.5283614351563615 16546.59541727944
...
# #EM-GMM 0.5508403250886906 28619.62391905364
print("----- EM-GMM -----")
start = time.time()
cluster_em = GaussianMixture(n_components=4)
